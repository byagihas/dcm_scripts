...
class gapi:
	def __init__( self, profile_id, advertiser_id = None,  store = API_STORE, version = API_VERSION, name = API_NAME, scope = API_SCOPE):
	...
	def getCampaigns( self):
		...
		request = service.campaigns().list( profileId = self.profileId)
		...
		try:
			...
			print('The credentials have been revoked or expired, please re-run the application to re-authorize')
        def getPlacements( self, campaign):
                service = api_con( self.scope, self.store, self.name, self.version)

                request = service.placements().list( profileId = self.profileId, advertiserIds=self.advertiserId, campaignIds=campaign)
